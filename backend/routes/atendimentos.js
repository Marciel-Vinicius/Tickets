...
router.get('/report/:date', async (req, res) => {
  ...
  try {
    ...
    doc.font('Helvetica-Bold').fontSize(10)
      ...
      .text('Loja', 160, startY, { width: 90 })
      .text('Contato', 260, startY, { width: 90 })
      .text('Ocorrência', 360, startY, { width: 200 });
    doc.moveDown(0.5);
    ...
    rowYs.forEach(row => doc.moveTo(40, row.y).lineTo(560, row.y).stroke());
    doc.moveTo(40, currentY).lineTo(560, currentY).stroke();
    const colEnds = [40, 100, 160, 260, 360, 560];
    colEnds.forEach(x => doc.moveTo(x, rowYs[0].y).lineTo(x, currentY).stroke());
